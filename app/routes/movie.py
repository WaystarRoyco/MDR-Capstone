...
from app.classes.data import Movie
from app.classes.forms import MovieForm
...
def movieNew():
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('movie',movieID=newMovie.id))
...
def movie(movieID):
    ...
    thisMovie = Movie.objects.get(id=movieID)
    # Send the blog object and the comments object to the 'blog.html' template.
    return render_template('movie.html',blog=thisMovie)
...
def movieList():
    ...
    # each blog.
    return render_template('movies.html',blogs=movies)

@app.route('/movie/edit/<movieID>', methods=['GET', 'POST'])
@login_required
def movieEdit(movieID):
    editMovie = Movie.objects.get(id=movieID)
    ...
    if current_user != editMovie.author:
        flash("You can't edit a movie entry you don't own.")
    ...
    form.title.data = editMovie.title
    form.director.data = editMovie.director
    form.rating.data = editMovie.rating
